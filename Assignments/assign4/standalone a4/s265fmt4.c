...
int fmt_commands(char * string, int * mrgn, int * pgwdth, int * fmt, int *first_time){
	...
	if (string[0]=='?') {
		...
		/*There was a question mark at beginning of string, but no valid format command*/
			...
			fprintf(stderr,"Invalid formatting command, check question marks. Exiting.\n");
			exit(1);
		}
...
int main(int argc, char *argv[]) {
	...
	char *infile_name;
	FILE *infile;
	/*Variables for array of string pointers, 
	current size, and max size respectively*/
	char **list;
	int count;
	/*Variable for size of new array and current
	size of array respectively*/
	char **new_list = NULL;

	int i=0;

	/*If there are more than two parameters, print error*/
	if (argc < 2) {
	...
	/*create dynamic array of pointers containing unformatted text*/
	list = create_array(infile,&count);	
	fclose(infile);
	/*
	i=0;
	while(i<count){
	...
	/*format the unformatted array*/
	new_list = format_lines(list,&count);
	
	/*print lines from formatted list*/
	for(i=0;i<count;i++){
		printf("%s\n",new_list[i]);
		free(new_list[i]);
	}
